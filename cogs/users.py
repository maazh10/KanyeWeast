...
class Users(commands.Cog):
    ...
    async def get_user(
        self, ctx: commands.Context, user: str | None
    ) -> discord.abc.User:
        if user == None:
    ...
    async def annoy(self, ctx: commands.Context, user=None, num_str=None, opt_str=None):
        ...
        annoy_string = (
            "get annoyed <:Pepepunch:794437891648520224>"
            if (opt_str == None)
            else " " + opt_str
        )
        if num == 1:
            ...
            for i in range(num):
                ...
                print(
                    f"{sleepnum}, {pinged.display_name}, {num - i}"
                ) if flag else print(f"{sleepnum}, {user}, {num - i}")
                await asyncio.sleep(sleepnum)
                if invalid_user or invalid_num or not in_range:
                    await ctx.send(
                        user.mention + annoy_string + " " + str(num - i),
                        delete_after=10,
                    )
                else:
                    await ctx.send(
                        user + annoy_string + " " + str(num - i), delete_after=10
                    )
